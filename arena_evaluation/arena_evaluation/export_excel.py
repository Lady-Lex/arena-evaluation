...
Current behavior:
- Read a single "data" directory that contains CSV files (e.g. metrics.csv,
  metrics_subject.csv, metrics_zone.csv, odom.csv), OR a directory that
  contains multiple run subdirectories each with those files.
- Export the aggregated rows into a single Excel sheet.
- Optional row coloring is applied only if a 'task' column exists.
"""
...
def _looks_like_run_dir(dir_path: str) -> bool:
    """Heuristic: does this directory contain at least one metrics file?"""
    for name in ("metrics.csv", "metrics_subject.csv", "metrics_zone.csv", "odom.csv"):
        if os.path.exists(os.path.join(dir_path, name)):
            return True
    return False


def collect_all_data(base_dir: str):
    """Collect data from a single run directory or from multiple run subdirectories."""
    all_data = []

    if _looks_like_run_dir(base_dir):
        print(f"Processing run directory: {base_dir}")
        metrics_data = load_metrics_data(base_dir)
        if metrics_data:
            metrics_data["run"] = os.path.basename(os.path.normpath(base_dir))
            all_data.append(metrics_data)
        return all_data

    # Otherwise treat base_dir as a container of multiple runs.
    print(f"Processing runs under: {base_dir}")
    try:
        children = sorted(
    ...
    for run_dir in children:
        ...
        metrics_data = load_metrics_data(run_dir)
        if not metrics_data:
            print(f"  Skipping {run_name}: no valid data")
            continue
        metrics_data["run"] = run_name
        all_data.append(metrics_data)
...
def apply_colors_to_excel(file_path, df):
    ...
    if task_col_idx is None:
        print("No 'task' column found, skipping color styling")
        return
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Export evaluation CSV metrics under a data directory into an Excel file")
    parser.add_argument(
        "-d",
        "--data-dir",
        default="data/tvss_nav",
        help="Run directory containing metrics.csv etc, or a directory containing many run subdirectories",
    )
    parser.add_argument(
        "-o",
        "--output",
        default=None,
        help="Output xlsx path (default: <data-dir>/metrics_<timestamp>.xlsx)",
    )
    args = parser.parse_args()

    base_dir = args.data_dir
    print(f"Start collecting data from: {base_dir}")
    ...
    if output_file is None:
        output_file = os.path.join(base_dir, f"metrics_{format_datetime()}.xlsx")
    output_file = create_excel_file(all_data, output_file=output_file)
